...
pub async fn download_file(
    ...
    let file_path = PathBuf::from("/Users/hk/Downloads/recording.mov");
    ...
    let response = match File::open(&file_path).await {
        Ok(file) => {
            ...
            //         println!("progress: {} ({}/{} bytes, {} chunks)", progress, sent_bytes, total_size, chunk_count);
            //         tauri::async_runtime::spawn({
            //             let app_handle = app_handle.clone();
            //             async move {
            //                 app_handle
            //                     .emit("file-download-progress", progress)
