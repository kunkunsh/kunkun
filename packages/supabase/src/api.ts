...
export class SupabaseAPI {
	...
	async getExtList(): Promise<SBExt[]> {
		...
		return dbExts
			.map((x) => {
				...
				return parsedNode.success
					? v.parse(SBExt, parsedNode.output)
					: null;
			})
			.filter((x) => x !== null);
	}
	...
	async incrementDownloads({
		...
		return this.supabase.functions
			...
			.then(({ data, error }) => {
				...
				if (!parsed.success) {
					throw new Error(
						"Fail to parse increment downloads response",
					);
				}
	...
	async publishExtFromJSR(
		...
		return this.supabase.functions.invoke("publish-jsr-ext", {
			body: payload,
	...
	translateExtensionFilePathToUrl(tarballPath: string): string {
		return this.supabase.storage.from("extensions").getPublicUrl(
			tarballPath,
		).data.publicUrl;
	}
}
